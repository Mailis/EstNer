...
function echoUpdateStats($dirname){
    ...
    $first5headers = array(0=>"process name", 1=>"start", 2=>"time spent (h:m:s.mm)", 3=>"nr of changes", 4=>"nr of workers");
    $nextheaders = array(0=>"action name", 1=>"start", 2=>"time spent (h:m:s.mm)");
    $firstHsize = count($first5headers);
    $nextHsize = count($nextheaders);
    $maxLIneLength = $firstHsize;
    if($filesarr != 0){
	    if(count($filesarr) > 0)
	    ...
	    if($lineArrLen > 0){
				...
				$rowsize = count($row);
                                if($row[($rowsize-1)]==" ")
                                   $rowsize--;
				if($rowsize > $maxLIneLength)
				   $maxLIneLength = $rowsize;
			    }
			    $nrRestOfHeadersGroup = ($maxLIneLength - $firstHsize)/$nextHsize;

		$tableHeaders = array_keys($lineArray[($lineArrLen-1)]);
		echo "<table class = 'update_table' border=1 ><tr>";
		foreach($first5headers as $key => $colhead){
                    if(strpos($colhead, "name") !== FALSE)
		        echo "<th class='updateTable'>" . $colhead . "</th>";
                    else
		        echo "<th>" . $colhead . "</th>";
		}
                for($i=0; $i < $nrRestOfHeadersGroup; $i++){
		    foreach($nextheaders as $keyi => $colheado){
                        if(strpos($colheado, "name") !== FALSE)
		            echo "<th class='updateTable'>" . $colheado . "</th>";
		        else
		            echo "<th>" . $colheado . "</th>";
		    }
		}
		...
		foreach($lineArray as $key => $row){
		    ...
		    foreach($row as $k => $cell){
                        $rowelemcounter++;
                        if($rowelemcounter == 1)
		            echo "<td class='updateTable'>" . implode(" ", explode("_", $cell)) . "</td>";
                        else if($rowelemcounter <= $firstHsize)
		            echo "<td>" . implode(" ", explode("_", $cell)) . "</td>";
                        else if (($rowelemcounter%$nextHsize) == 0)
		            echo "<td class='updateTable'>" . implode(" ", explode("_", $cell)) . "</td>";
                        else
		            echo "<td>" . implode(" ", explode("_", $cell)) . "</td>";
		    }
...
function readUPDfile($filename){
	...
	while (($line = fgets($handle)) !== false) {
		...
		$lineSplitted = explode(" ", $line);
                foreach($lineSplitted as $key => $value){
                    $linedata[] = $value;
                }
                $lineArray[] = $linedata;
        }
