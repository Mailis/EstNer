...
def getRDFgraph(g, gType):
    rdfDir = "../rdf_files/"
    filePath = rdfDir + "LOC.rdf"
    if(gType == "org"):
        filePath = rdfDir + "ORG.rdf"
    elif(gType == "per"):
        filePath = rdfDir + "PER.rdf"
    try:
        g.parse(filePath)
    except:
        e = sys.exc_info()[0]
...
'''
try:
    g.parse("../rdf_files/PER.rdf")#../#data2.rdf
except:
    e = sys.exc_info()[0]
    print('Error: %s' % e )
'''
